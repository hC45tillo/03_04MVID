...
void render(const Geometry& floor, const Geometry& tetera, const Geometry& light, const Shader& s_phong, const Shader& s_light,
	const Texture& t_albedo, const Texture& t_specular, float dt) {
	...
	glm::vec3 lightPos(glm::vec3(glm::cos(static_cast<float>(dt)), 1,glm::sin(static_cast<float>(dt))));
...
int main(int, char* []) {
	...
	while (window->alive()) {
		...
		render(floor, tetera, sphere, s_phong, s_light, t_albedo, t_specular, currentFrame);
		window->frame();
	}
