...
void render(const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light) {
	...
	glm::vec3 lightDiffuse(.5f, .5f, 1.0f);
	...
	s_phong.set("material.ambient", 0.25f,	0.25f,	0.25f);
	s_phong.set("material.diffuse", 0.4f,	0.4f,	0.4f);
	s_phong.set("material.specular", 0.774597,	0.774597,	0.774597);
	s_phong.set("material.shininess", 132);

	s_phong.set("light.position", lightPos);
	s_phong.set("light.ambient", 1.0f, 0.5f, 0.1f);
	s_phong.set("light.diffuse", lightDiffuse);
	s_phong.set("light.specular", 1.0f, 0.0f, 0.5f);
...
int main(int, char* []) {
	...
	const Sphere sphere(0.4f, 50, 50);
	...
	while (window->alive()) {
		...
		render(cube, sphere, s_phong, s_light);
		window->frame();
	}
