...
void render(const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light) {
	...
	lightAngle += 1;
	lightPos.z = lightRadius * cos(glm::radians(lightAngle));
	lightPos.x = lightRadius * sin(glm::radians(lightAngle));
...
int main(int, char* []) {
    ...
    while (window->alive()) {
		...
		//lightPos.x = glfwGetTime() * lightPos.x;
		//lightPos.z = glfwGetTime() * lightPos.z;
