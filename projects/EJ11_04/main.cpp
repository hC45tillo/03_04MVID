...
void render(const Geometry& quad, const Shader& s_phong, const Shader& s_blend, const Texture& t_albedo, const Texture& t_specular) {
	...
	for (int i = 0; i < 5; i++) {
		if (i % 2 == 0) {
			...
			s_blend.set("material.ambient", colors[i]);
			s_blend.set("material.diffuse", colors[i]);
			s_blend.set("material.specular", colors[i]);
			s_blend.set("material.shininess", 128);

			s_blend.set("material.alpha", 0.5f);
...
int main(int, char* []) {
	...
	while (window->alive()) {
		...
		render(quad, s_phong, s_blend, t_albedo, t_specular);
		window->frame();
	}
